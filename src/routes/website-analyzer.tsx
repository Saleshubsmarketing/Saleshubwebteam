...
export const Route = createFileRoute("/website-analyzer")({
  head: () => ({
    meta: [
      { title: "AI Website Analyzer — Free Shopify Audit | SaleshubsWebOffice" },
...
function buildReport(url: string, a: Audit) {
  const lines: string[] = [];
  lines.push(`# SaleshubsWebOffice — Website Audit`);
  ...
  lines.push(`---`, ``, `Generated by SaleshubsWebOffice · novacommerce.agency`);
  return lines.join("\n");
}